...
You can now type Valkey command.

For example `KEYS *` which output the list of keys with any name in the base. In this empty database, there should be none.
...
![Opening the Valkey CLI in the Valkey Container](./pics/partial-display-when-using-valkey-cli.png)
...
You can know click on the local database (with alias `valkey:6379` on the screenshot - the alias has not been changed in the previous step) to access this base.
...
If you want to experiment with Valkey but do not readily have available data, that can be a good way to start.
